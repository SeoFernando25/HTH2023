import { createUserLocalInfo, serverInfoToUserInfo, userInfoToServerInfo } from "./browserCrypt";
 ...
 */
export async function tryLogin(username: string, password: string): Promise<UserLocalInfo | null> {
    ...
    switch (res.status) {
        case 200: {
            ...
            const userInfo = await serverInfoToUserInfo(serverUserInfo, username, password);
            if (userInfo) {
                return userInfo;
            } else {
...
export async function tryRegister(username: string, password: string): Promise<UserLocalInfo> {
